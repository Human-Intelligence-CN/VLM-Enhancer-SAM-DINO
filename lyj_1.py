...
CONFIG = {
    ...
    "image_path": "/root/autodl-tmp/data/images/img-1.jpg",        
    "output_path": "/root/autodl-tmp/output/output_res.png", 
    
    "batch_input_dir": "/root/autodl-tmp/data/images", 
    "batch_output_dir": "/root/autodl-tmp/output",             
    
    "gsd": 0.015,  
    "dino_classes": ['a dark deep hole on the road', 'broken asphalt hole', 'distinct road surface crack'], 
    "box_threshold": 0.30,  
    "text_threshold": 0.30, 
    "prompt_step1": "",
}
...
# [核心系统类]
# =====================================================================
class RoadDamagePerceptionSystem:
    ...
    def step2_open_vocabulary_localization(self, image_path, verbose=True):
        ...
        for i in range(len(detections.xyxy)):
            ...
            phrase = self.cfg["dino_classes"][cls_id] if cls_id is not None else "Unknown Damage"
    ...
    def step4_final_report(self, image_pil, initial_desc, area, length, verbose=True):
        prompt = (f"Based on visual observation: '{initial_desc}', and SAM 2 measurements: Area = {area:.2f} m², "
                  f"Max Length = {length:.2f} m. Write a professional assessment report explicitly stating these numbers.")
        image_tensor = self.ib_vis_processors["eval"](image_pil).unsqueeze(0).to(self.device)
        return self.ib_model.generate({"image": image_tensor, "prompt": prompt}, max_length=512, min_length=50, repetition_penalty=1.5)[0]

    def visualize_and_save(self, image_pil, bbox, mask, area, length, out_path):
        ...
        plt.gca().text(bbox[0], bbox[1] - 10, f"Area: {area:.2f} m²\nLength: {length:.2f} m", color='white', weight='bold', bbox=dict(facecolor='red', alpha=0.5))
    ...
    def generate_research_dashboard(self, df, output_dir):
        ...
        # NPG (Nature Publishing Group) 经典配色体系
        npg_palette = ["#E64B35", "#4DBBD5", "#00A087", "#3C5488", "#F39B7F", "#8491B4"]
        
        fig, axes = plt.subplots(1, 3, figsize=(20, 6))
        
        # 1. 病害类型占比饼图
        type_counts = df['Damage Type'].value_counts()
        ...
        axes[0].set_title('Distribution of Road Damage Types', fontweight='bold')
        
        # 2. 物理面积分布直方图
        sns.histplot(data=df, x='Area (m²)', kde=True, ax=axes[1], color=npg_palette[1])
        axes[1].set_title('Distribution of Damaged Area', fontweight='bold')
        axes[1].set_xlabel('Area (Square Meters)')
        axes[1].set_ylabel('Image Count')
        
        # 3. 置信度 vs 面积散点图
        sns.scatterplot(data=df, x='Area (m²)', y='Confidence', hue='Damage Type', palette=npg_palette[:len(type_counts)], ax=axes[2], s=100, alpha=0.8)
    ...
    def generate_top_cases_grid(self, df, output_dir):
        ...
        fig.suptitle('Critical Road Damages (Top Area Analysis)', fontsize=20, fontweight='bold', y=1.02)
        ...
        for idx, (_, row) in enumerate(top_cases.iterrows()):
            ...
            if os.path.exists(res_path):
                ...
                axes[idx, 1].set_title(f"Processed: Area {row['Area (m²)']:.2f} m² | Type: {row['Damage Type']}", fontsize=14)
            axes[idx, 1].axis('off')
    ...
    def batch_process_directory(self):
        ...
        if not image_files: return print(f"[警告] {input_dir} 未找到图片。")
            
        print(f"\n[🚀] 开始批量处理 {len(image_files)} 张图像...")
        
        records = [] # 用于保存 Excel 数据的列表
        ...
        for img_path in tqdm(image_files, desc="Processing Images"):
            ...
            try:
                ...
                if bbox is not None:
                    ...
                    self.visualize_and_save(image_pil, bbox, mask, area, length, out_img_path)
        ...
        if records:
            ...
            excel_path = os.path.join(output_dir, "Summary_Report.xlsx")
            # 删除用于绘图的临时路径列，再保存给用户看
            df.drop(columns=['Original_Path']).to_excel(excel_path, index=False)
...
if __name__ == "__main__":
    system = RoadDamagePerceptionSystem(CONFIG)
